# Script 2 ----------------------------------------------------------------
# Figure 2 and Supplemental Figure 2A
...
map <- read_excel("/Users/mhswaney/Lab_data_permanent/Skin_media_project/DataAnalysis/File_Mapping_GrowthCurves.xlsx", sheet = "Sweat")
...
# color palette for sweat concentration
...
ReadSweat <- function(mapping_file, type){
  ...
  # assign sweat concentrations
  gathered$Concentration <- ifelse(startsWith(gathered$Well,"A"), "0X", 
                                   ifelse(startsWith(gathered$Well,"B"), "0.125X",
                                          ifelse(startsWith(gathered$Well,"C"), "0.25X",
                                                 ifelse(startsWith(gathered$Well,"D"), "0.5X",
                                                        ifelse(startsWith(gathered$Well,"E"), "1X",
                                                               ifelse(startsWith(gathered$Well,"F"), "2X",
                                                                      ifelse(startsWith(gathered$Well,"G"), "3X",
                                                                             ifelse(startsWith(gathered$Well,"H"), "4X", NA))))))))
  ...
  # exclude wells due to contamination
  gathered <- gathered %>% filter(!((Date == "2022_1_11" & Well == "E6" & Strain == "Dermabacter hominis LK522") | (Date == "2022_2_10" & Well == "E6" & Strain == "Dermacoccus nishinomiyaensis LK1128")))
  gathered <- gathered %>% filter(!((Date == "2022_4_22" & Well == "F9")))
...
# Read in sweat growth curves and bind data together
DataList <- apply(map, 1, FUN = ReadSweat)
...
samples_avg$Concentration <- factor(samples_avg$Concentration, levels = c("0X","0.125X","0.25X","0.5X","1X","2X","3X","4X"))
...
# Plot sweat growth curves - Figure 2A
sweat_big <- samples_avg %>%
  ...
  labs(color="Sweat\nconcentration",linetype="Sweat\nconcentration") + 
  xlab("Time (hours)") + ylab("OD600") + 
  scale_linetype_manual(values=c("0X"=1,"0.125X"=2,"0.25X"=1,"0.5X"=2,"1X"=1,"2X"=2,"3X"=1,"4X"=2)) +
  scale_fill_manual(values=rainbow, guide="none") + 
  ylim(-0.1,1.3)

#ggsave(plot=sweat_big, filename="/Users/mhswaney/Kalan_Lab/Manuscripts/SkinMedia/Figures/Figure2/SweatCurves2.pdf",
       #width = 12, height=10, units = "in", device = "pdf")

# Area Under the Curve ----------------------------------------------------

conc <- c("0X","0.125X","0.25X","0.5X","1X","2X","3X","4X")
strain <- unique(DataAllStrains$Strain)
strain <- strain[which(strain != "NA")]
strain <- strain[which(strain != "Blank")]

# Calculate area under the curve for each sweat concentration for each strain
AUC <- lapply(strain, FUN=function(X){ # loop through each strain
  strain_filter <- samples_avg %>% filter(Strain == X) 
  areas <- lapply(conc, FUN=function(Y){ # loop through each strain concentration
    conc_filter <- strain_filter %>% filter(Concentration == Y)
    fit <- loess(conc_filter$avg_OD600~conc_filter$Time) # Fit LOESS model for the biological replicates
    df <- data.frame(x = conc_filter$Time)
    df <- transform(df, y.pred = predict(fit, df)) # predict y values using model (same result as geom_smooth curves above)
    auc <- pracma::trapz(df$x, df$y.pred) # calculate area under the curve
  })
  names(areas) <- conc
  return(areas)
})
...
# combine all data for each strain and sweat concentration
...
# Plot sweat AUC - Figure 2B
auc_plot <- auc_df %>% 
  ggplot(aes(x=Concentration, y=AUC, fill=Concentration)) + 
  theme_bw() + 
  geom_col(color="black") + 
  facet_wrap(~Strain) + scale_fill_manual(values = rainbow) + 
  theme(axis.text.x = element_text(angle=45, hjust=1),
        text=element_text(size=5),
        legend.position = "none") + xlab("Sweat concentration (%)")

#ggsave(plot=auc_plot, filename="/Users/mhswaney/Kalan_Lab/Manuscripts/SkinMedia/Figures/Figure2/SweatAUC.pdf",
       #width = 5.45, height=5, units = "in", device = "pdf")

# Linear Regression -------------------------------------------------------

# normalize AUC data out of 1
auc_df <- auc_df %>% separate(Concentration, into = c("Num_Concentration","X"), sep= "X",remove = FALSE)
...
# linear regression with AUC data for each strain, exclude 4X sweat because not many strains grew
fitted_models <- auc_df %>% filter(Concentration != "4X") %>% 
  group_by(Strain) %>% 
  do(model=lm(NormAUC~as.numeric(Num_Concentration), data = .))
...
auc_point <- ggplot(auc_lm, aes(x=Coef, y=-log10(Pval), color = Genus)) + 
  ...
  annotate("text", x=-0.16, y=4.5, label = "Low sweat") +
  annotate("text", x=0.16, y=4.5, label = "High sweat") + 
  annotate("text",x=-0.2, y=0.5, label = "No preference")  + 
  theme(legend.position = "none")

# classify sweat preferences
auc_lm$SweatPref <- ifelse(auc_lm$Pval < 0.05 & auc_lm$Coef < 0, "Low sweat",
                           ifelse(auc_lm$Coef > 0 & auc_lm$Pval < 0.05, "High sweat", "No preference"))
auc_lm$SweatPref <- factor(auc_lm$SweatPref, levels = c("No preference","High sweat","Low sweat"))

# preference plot for Figure 2C
pref_auc <- ggplot(auc_lm, aes(x=SweatPref, fill=Genus)) + geom_bar(color="black") + 
  theme_bw() +
  scale_fill_manual(values=cols) +
  scale_x_discrete(drop=FALSE) + 
  xlab("Sweat preference predicted by AUC") + 
...
#ggsave(pref_auc, filename = "/Users/mhswaney/Kalan_Lab/Manuscripts/SkinMedia/Figures/Figure2/SweatPrefLM.pdf",
     #  width = 3, height=4, units = "in", device = "pdf")

#ggsave(auc_point, filename = "/Users/mhswaney/Kalan_Lab/Manuscripts/SkinMedia/Figures/Figure2/SweatLMPoint.pdf",
...
maxgrowth$Concentration <- factor(maxgrowth$Concentration, levels = c("0X","0.125X","0.25X","0.5X","1X","2X","3X","4X"))
...
growthrate <- ggplot(maxgrowth, aes(x=as.factor(Concentration), y=mumax, color=as.factor(Concentration))) + geom_point(size=2.5) + 
  facet_wrap(~Strain) + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle=45, hjust=1),
        text=element_text(size=5)) + 
  xlab("Sweat Concentration") + ylab("Maximum growth rate") +
  scale_color_manual(values=rainbow)

#ggsave(plot=growthrate, filename="/Users/mhswaney/Kalan_Lab/Manuscripts/SkinMedia/Figures/Figure2/SweatGrowthRate.pdf",
